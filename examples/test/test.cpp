...
void write_buff_to_file(char* filename, const char* buff, int bsize, long offset) {
...
char* gen(char* filename, uint8_t* pub, int pubSize, uint8_t* prv, int prvSize, uint8_t* scratchBuffer, int scratchSize) {
    ...
    encryption = ecall_encrypt(&pub, pubSize, scratchBuffer, scratchSize, &fileBuffer, fileSize, &enSize);
    ...
    decryption = ecall_decryption(&prv, prvSize, scratchBuffer, scratchSize, &encryption, enSize, &deSize);
...
int main(void)
{
    int pubSize = ecall_get_pubSize();
    printf("pubSize: %d\n", pubSize);

    uint8_t* pub = ecall_gen_pubKey(&pubSize);
    printf("pub:  %s\n", pub);

    int prvSize = ecall_get_prvSize();
    printf("prvSize: %d\n", prvSize);

    uint8_t* prv = ecall_gen_prvKey(&prvSize);
    printf("prv:  %s\n", prv);

    int scratchSize = ecall_gen_scratchSize(&prv, &pub, prvSize, pubSize);
    ...
    while(yelpPhoto != NULL && i < n * 5000) {
        printf("i:%d\n", i);
        
        if (i >= ((n - 1) * 5000)) {
            // result = char_concat(result, char_concat(yelpPhoto, "\n"));
            result = std::string(result) + std::string(yelpPhoto) + std::string("\n");
            printf("result: %s\n", result);
            // result = char_concat(result, char_plus_int("size:", yelpSize));
            result = std::string(result) + std::string("size:") + std::to_string(yelpSize) + std::string("\n");
            printf("result: %s\n", result);
            ...
            result = std::string(result) + std::string(temp);
        }
    ...
    char const *c = result.c_str();
    write_buff_to_file("/home/zyktrcn/sgx/ipp-crypto/examples/_build/.build/examples/result.txt", c, strlen(c), 0);
